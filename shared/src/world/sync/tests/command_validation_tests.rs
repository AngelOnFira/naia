...
use crate::{
    ...
    GlobalEntity, HostEntity, RemoteEntity, HostType, EntityAuthStatus,
    ComponentKind, EntityCommand, BigMapKey,
};
use std::any::TypeId;
...
fn test_all_migration_commands_through_real_flow() {
    ...
    assert!(commands.len() >= 2, "Should have at least 2 commands buffered");
...
fn test_request_authority_command() {
    ...
    let commands = host_channel.extract_outgoing_commands();
    let has_request = commands.iter().any(|cmd| {
        matches!(cmd, EntityCommand::RequestAuthority(_, _))
    });
    assert!(has_request, "RequestAuthority should be in outgoing commands");
}
...
fn test_enable_delegation_response_command() {
    ...
    let commands = host_channel.extract_outgoing_commands();
    let has_response = commands.iter().any(|cmd| {
        matches!(cmd, EntityCommand::EnableDelegationResponse(_, _))
    });
    assert!(has_response, "EnableDelegationResponse should be in outgoing commands");
}
