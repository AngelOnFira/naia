...
use naia_shared::{
    BigMapKey, ComponentKind, EntityAuthStatus, GlobalEntity, HostType,  
    LocalWorldManager, OwnedLocalEntity, RemoteEntity,
};
...
fn migration_preserves_component_state() {
    ...
    assert!(
        local_world_manager.get_remote_entity_auth_status(&global_entity).is_some(),
...
fn migration_installs_entity_redirects() {
    ...
    local_world_manager.insert_remote_entity(&global_entity, RemoteEntity::new(200), HashSet::new());
...
fn multiple_entity_migrations() {
    ...
    for (global_entity, _) in &entities {
        assert_eq!(
            ...
            "Entity {:?} should have Granted authority", global_entity
        );
    }
    ...
    for (global_entity, _) in &entities {
        ...
        assert_eq!(
            local_world_manager.get_remote_entity_auth_status(global_entity),
            Some(EntityAuthStatus::Granted),
            "Entity {:?} should regain authority independently", global_entity
        );
    }
}
