...
use naia_shared::{
    BigMapKey, GlobalEntity, HostType, LocalWorldManager, RemoteEntity,
};
...
fn server_migrate_response_uses_wrong_entity_ids() {
    ...
    println!("SERVER: Client entity GlobalEntity({}) = RemoteEntity({})",
        BigMapKey::to_u64(&global_entity), server_remote_entity.value());
    ...
    if result.is_err() {
        println!("❌ BUG CAUGHT: Server cannot reserve HostEntity for GlobalEntity that already exists!");
...
fn client_cannot_look_up_servers_remote_entity() {
    ...
    println!("CLIENT: Created entity - GlobalEntity({}) = HostEntity({})",
        BigMapKey::to_u64(&global_entity), client_host_entity.value());
    ...
    println!("\nSERVER SENDS: MigrateResponse with RemoteEntity({})", server_remote_entity.value());
    println!("  (This is the SERVER's local ID for the client's entity)");
    
    // Client tries to look up this RemoteEntity
    println!("\nCLIENT: Attempting to look up RemoteEntity({})...", server_remote_entity.value());
    
    let lookup_result = client_lwm.entity_converter()
    ...
    match lookup_result {
        ...
        Err(_) => {
            println!("❌ BUG CONFIRMED: Client cannot find RemoteEntity({}) in its entity_map!",
                server_remote_entity.value());
            println!("   The client only knows about HostEntity({}).", client_host_entity.value());
            println!("   The server sent the wrong entity ID!");
            println!("\n   CORRECT BEHAVIOR:");
            println!("   - Server should send the CLIENT's HostEntity({})", client_host_entity.value());
            println!("   - NOT the server's RemoteEntity({})", server_remote_entity.value());
        }
    }
}
    ...
    match lookup_result {
        Ok(ge) => {
            println!("\n✓ SUCCESS: Client found HostEntity({}) -> GlobalEntity({})",
                client_host_entity.value(), BigMapKey::to_u64(&ge));
