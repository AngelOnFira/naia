...
use crate::{events::InsertBundleEvent, bundle::ReplicateBundle, events::{InsertComponentEvent, RemoveComponentEvent, UpdateComponentEvent}, component_event_registry::ComponentEventRegistry};
...
impl AppRegisterComponentEvents for App {
    ...
    fn add_bundle_events<B: ReplicateBundle>(&mut self) -> &mut Self {
        ...
        // add component type to registry
        let mut component_event_registry = self.world_mut().resource_mut::<ComponentEventRegistry>();
        component_event_registry.bundle_registry_mut().register_bundle_handler::<B>();
